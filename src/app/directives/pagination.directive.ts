...
export class StylePaginatorDirective implements DoCheck, AfterViewInit {
    ...
    constructor(
      ...
      this.matPag._intl.getRangeLabel = (page: number, pageSize: number, length: number): string => {
        ...
        const endIndex = startIndex < length ?
          ...
          startIndex + pageSize;
        return length > 0 ? 'Hiển thị ' + (startIndex + 1) + ' – ' + endIndex + ' trên ' + length + ' bản ghi' : 'Hiện thị 0 – 0 trên 0 bản ghi';
      };
    ...
    private createButton(index: string, pageIndex: number): MatButton {
      ...
      this.renderer.addClass(linkBtn, 'btn');
      this.renderer.addClass(linkBtn, 'btn-primary');
      this.renderer.addClass(linkBtn, 'mx-1');
      switch (index) {
        case `${pageIndex}`:
